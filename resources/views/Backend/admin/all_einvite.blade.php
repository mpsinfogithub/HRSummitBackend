                      ...
                      <div class="col-sm-12">
                        <div class="card-body">
                          <div class="breadcrumb-body">
                            <h5 class="card-title text-primary">All e-invite</h5>
                            <nav aria-label="breadcrumb">
                              <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="{{url('admin/dashboard')}}">Home</a></li>
                                <li class="breadcrumb-item active">All e-invite</li>
                              </ol>
                            </nav>
                          </div>
                          <a href="{{url('admin/add-einvite')}}" class="btn btn-success btn-alignment"><i class='bx bxs-plus-circle'></i>&nbsp;Add</a>
                          <br>
                          <div class="card">
                <div class="table-responsive text-nowrap">
                  <table class="table table-striped text-center" id="myTable">
                    ...
                    <tbody class="table-border-bottom-0">
                      @if(isset($all_einvite[0]))
                        @foreach($all_einvite as $row)
                            <tr>
                                <td>{{ $count++ }}</td>
                                <td>
                                ...
                                <td>
                                @if($row->status == 1)
                                                                <a href="{{url('admin/change-einvite-status/'.'0'.'/'.$row->id)}}" 
                                                            ...
                                                            @else
                                                                <a href="{{url('admin/change-einvite-status/'.'1'.'/'.$row->id)}}" 
                                                            ...
                                                            <a href="{{url('admin/edit-einvite/'.$row->id)}}" class="item item-btn" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" data-bs-html="true" title="" data-bs-original-title="<span>Edit</span>">
                                                                <i class='bx bxs-pencil text-warning'></i>
                                                                </a> &nbsp;
                                                                
                                                            <a href="{{url('admin/delete-einvite/'.$row->id)}}" 
